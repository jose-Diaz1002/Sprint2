...
SELECT p.apellido1, p.apellido2, p.nombre, d.nombre AS nombre_departamento FROM persona p JOIN profesor pr ON p.id = pr.id_profesor JOIN departamento d ON pr.id_departamento = d.id ORDER BY p.apellido1, p.apellido2, p.nombre;
...
SELECT a.nombre, cs.anyo_inicio, cs.anyo_fin FROM persona p JOIN alumno_se_matricula_asignatura am ON p.id = am.id_alumno JOIN asignatura a ON am.id_asignatura = a.id JOIN curso_escolar cs ON am.id_curso_escolar = cs.id WHERE p.nif = '26902806M';
...
SELECT a.id, a.nombre AS asignatura, p.nombre, g.nombre  FROM asignatura a JOIN grado g ON g.id = a.id_grado JOIN profesor pr ON a.id_profesor = pr.id_profesor JOIN persona p ON p.id = pr.id_profesor where  g.nombre = 'Grado en Ingeniería Informática (Plan 2015)' AND a.id_profesor IS NOT NULL;
...
SELECT DISTINCT p.nombre FROM persona p JOIN alumno_se_matricula_asignatura asma ON p.id = asma.id_alumno JOIN curso_escolar ce ON asma.id_curso_escolar = ce.id WHERE ce.anyo_inicio = '2018' AND ce.anyo_fin = '2019';
...
SELECT d.nombre, p.apellido1, p.apellido2, p.nombre FROM persona p LEFT JOIN profesor pr ON p.id = pr.id_profesor LEFT JOIN departamento d ON d.id = pr.id_departamento ORDER BY  d.nombre,p.apellido1,p.apellido2, p.nombre ASC; 
SELECT d.nombre, p.apellido1, p.apellido2, p.nombre FROM departamento d RIGHT JOIN profesor pr ON d.id = pr.id_departamento RIGHT JOIN persona p ON p.id = pr.id_profesor ORDER BY d.nombre ASC, p.apellido1 ASC, p.apellido2 ASC, p.nombre ASC;

-- 2.Devuelve un listado con los profesores/as que no están asociados a un departamento.
SELECT d.nombre,p.apellido1,p.apellido2, p.nombre FROM persona p LEFT JOIN profesor pr ON p.id = pr.id_profesor LEFT JOIN departamento d ON d.id = pr.id_departamento WHERE pr.id_departamento IS NULL ORDER BY  d.nombre,p.apellido1,p.apellido2, p.nombre ASC;
SELECT d.nombre,p.apellido1,p.apellido2, p.nombre FROM departamento d RIGHT JOIN profesor pr ON pr.id_departamento = d.id RIGHT JOIN persona p ON p.id = pr.id_profesor WHERE pr.id_departamento IS NULL ORDER BY  d.nombre,p.apellido1,p.apellido2, p.nombre ASC;
 
-- 3.Devuelve un listado con los departamentos que no tienen profesores asociados.
SELECT d.nombre, pr.id_profesor FROM departamento d LEFT JOIN profesor pr ON d.id = pr.id_departamento WHERE pr.id_profesor IS NULL;
SELECT d.nombre, pr.id_profesor FROM profesor pr RIGHT JOIN departamento d ON d.id = pr.id_departamento WHERE pr.id_profesor IS NULL;

-- 4.Devuelve un listado con los profesores/as que no imparten ninguna asignatura.
SELECT p.nombre, a.nombre FROM persona p LEFT JOIN profesor pr ON p.id = pr.id_profesor LEFT JOIN asignatura a ON pr.id_profesor = a.id_profesor WHERE a.nombre IS NULL;
SELECT p.nombre, a.nombre FROM asignatura a  RIGHT JOIN profesor pr ON a.id_profesor = pr.id_profesor RIGHT JOIN persona p ON pr.id_profesor = p.id WHERE a.nombre IS NULL;

-- 5.Devuelve un listado con las asignaturas que no tienen un profesor/a asignado.
SELECT a.nombre, pr.id_profesor FROM asignatura a LEFT JOIN profesor pr ON a.id_profesor = pr.id_profesor WHERE a.id_profesor IS NULL;
SELECT a.nombre, pr.id_profesor FROM profesor pr RIGHT JOIN asignatura a  ON a.id_profesor = pr.id_profesor WHERE a.id_profesor IS NULL;

-- 6.Devuelve un listado con todos los departamentos que no han impartido asignaturas en ningún curso escolar.
SELECT DISTINCT d.nombre AS departamento, a.nombre, ce.anyo_inicio FROM departamento d LEFT JOIN profesor pr ON d.id = pr.id_departamento LEFT JOIN asignatura a ON pr.id_profesor = a.id_profesor LEFT JOIN alumno_se_matricula_asignatura asma ON a.id = asma.id_asignatura LEFT JOIN curso_escolar ce ON asma.id_curso_escolar = ce.id WHERE a.id IS NULL ORDER BY d.nombre;
SELECT DISTINCT d.nombre AS departamento, a.nombre, ce.anyo_inicio FROM curso_escolar ce RIGHT JOIN alumno_se_matricula_asignatura asma ON asma.id_curso_escolar = ce.id RIGHT JOIN asignatura a ON a.id = asma.id_asignatura RIGHT JOIN profesor pr ON pr.id_profesor = a.id_profesor RIGHT JOIN departamento d  ON d.id = pr.id_departamento WHERE a.id IS NULL ORDER BY d.nombre;
...
SELECT d.nombre AS departamento, COUNT(*) AS numero_profesores FROM profesor pr JOIN departamento d ON pr.id_departamento = d.id GROUP BY d.nombre ORDER BY numero_profesores DESC;
...
SELECT g.nombre AS nombre_grado, COUNT(a.id) AS numero_asignaturas FROM grado g LEFT JOIN asignatura a ON g.id = a.id_grado GROUP BY g.nombre HAVING COUNT(a.id) > 40;
...
SELECT g.nombre AS nombre_grado, a.tipo, SUM(a.creditos) AS total_creditos FROM asignatura a JOIN grado g ON a.id_grado = g.id  GROUP BY g.nombre, a.tipo;
...
SELECT ce.anyo_inicio, COUNT(DISTINCT asma.id_alumno) AS numero_alumnos FROM alumno_se_matricula_asignatura asma JOIN curso_escolar ce ON asma.id_curso_escolar = ce.id GROUP BY ce.anyo_inicio;
...
SELECT p.id, p.nombre, p.apellido1, p.apellido2, COUNT(a.id) AS numero_asignaturas FROM persona p JOIN profesor pr ON p.id = pr.id_profesor LEFT JOIN asignatura a ON pr.id_profesor = a.id_profesor WHERE p.tipo = 'profesor' GROUP BY p.id, p.nombre, p.apellido1, p.apellido2 ORDER BY numero_asignaturas DESC;
...
SELECT p.nombre, p.apellido1, p.apellido2 FROM persona p JOIN profesor pr ON p.id = pr.id_profesor LEFT JOIN asignatura a ON pr.id_profesor = a.id_profesor WHERE p.tipo = 'profesor' AND pr.id_departamento IS NOT NULL AND a.id IS NULL;
